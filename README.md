...
```bash
npm install -g bitgreen-node
bitgreen-node create mynode
cd mynode
bitgreen-node install insight-api
bitgreen-node start  # to also start the service
```
...
- [bitgreen Node Dash 4.x](https://github.com/bitgreen/bitgreen-node)
...
To protect the server, insight-api has a built-in query rate limiter. It can be configurable in `bitgreen-node.json` with:
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in bitgreend.
- The endpoint for `/peer` is no longer relevant connection to bitgreend is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking bitgreend.
